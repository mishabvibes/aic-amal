...
export default function OngoingCampaignsPage() {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      setIsLoading(true);
      try {
        const response = await fetch("/api/campaigns/admin?status=active"); // Fetch only active campaigns
              ...
              >
                <div className="h-48 bg-gradient-to-br from-emerald-500/20 to-blue-500/20 relative">
                  {campaign.featuredImage && (
                    <img
                      src={
                        typeof campaign.featuredImage === "string"
                          ? campaign.featuredImage // If API returns a data URL
                          : `data:${campaign.featuredImageType || "image/jpeg"};base64,${Buffer.from(campaign.featuredImage.buffer).toString("base64")}` // If API returns a Buffer
                      }
                      alt={campaign.name}
...
//     if (confirm(`Are you sure you want to delete campaign "${name}"?`)) {
//       try {
//         const response = await fetch(`/api/campaigns/admin/${id}`, {
//           method: "DELETE",
//         });
//         if (response.ok) {
//           setCampaigns(campaigns.filter(c => c._id !== id));
...
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//           {filteredCampaigns.map((campaign) => {
//             const progress = Math.round((campaign.currentAmount / campaign.goal) * 100);
//             const daysRemaining = calculateDaysRemaining(campaign.endDate);
//             return (
//               <div 
//                 key={campaign._id}
...
//                         <Users className="h-3 w-3 mr-1" /> Donors
//                       </div>
//                       <p className="font-semibold text-gray-800 dark:text-white">N/A</p> {/* Update if donor count is added */}
//                     </div>
//                     <div className="bg-white/20 dark:bg-gray-800/20 rounded-lg p-3 text-center">
//                       <div className="flex items-center justify-center text-xs text-gray-500 dark:text-gray-400 mb-1">
//                         <Heart className="h-3 w-3 mr-1" /> Raised
//                       </div>
//                       <p className="font-semibold text-emerald-600 dark:text-emerald-400">₹{campaign.currentAmount.toLocaleString()}</p>
//                       <p className="text-xs text-gray-500 dark:text-gray-400">of ₹{campaign.goal.toLocaleString()}</p>
//                     </div>
//                   </div>
//                   <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-2 mb-4">
//                     {campaign.description}
...
//                 <div className="p-4 pt-0 flex justify-between">
//                   <Link 
//                     href={`/campaigns/track?id=${campaign._id}`}
//                     className="px-3 py-2 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 text-sm font-medium hover:bg-white/20 transition-all duration-300 flex items-center"
//                   >
//                     <BarChart2 className="h-4 w-4 mr-2" /> Track Progress
//                   </Link>
