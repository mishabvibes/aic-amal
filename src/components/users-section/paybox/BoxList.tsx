...
const BoxList = ({ boxes, userData, onBoxSelect, onEditProfile }: BoxListProps) => {
  ...
  //     try {
  //       console.log('Fetching with phone:', userData.phoneNumber);

  //       const response = await fetch(`/api/boxes/totalamount?phone=${encodeURIComponent(userData.phoneNumber)}`, {
  //         method: 'GET',
  //         headers: {
  ...
  // Helper function to get amount for a box by its ID
  const getBoxAmount = (boxId: string): number => {
    const boxAmount = data?.amountsByBox.find(amount => amount.boxId === boxId);
    return boxAmount ? boxAmount.amount : 0;
  };
        ...
        <div className="bg-white rounded-lg border border-indigo-100 p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-indigo-900">{boxes.length}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {boxes.filter(box => box.status === 'active').length}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">
                {boxes.filter(box => box.status === 'overdue').length}
              </p>
            </div>
