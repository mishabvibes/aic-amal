...
export function CampaignCard({ campaign, isActive, onClick }: CampaignCardProps) {
  ...
  const getGoalLabel = (type: string, goal: number | null | undefined, isInfinite: boolean) => {
    if (isInfinite) {
      // For infinite campaigns, just return "Infinite" without accessing goal
      return type === "physical" ? "Infinite Items" : "Infinite Amount";
    } else {
      // For non-infinite campaigns, ensure goal is defined before formatting
  ...
  // const getGoalLabel = (type: string, goal: number, isInfinite: boolean) => {
    
  //   return type === "physical" ? `${goal.toLocaleString()} items${isInfinite ? " (Infinite)" : ""}` : `₹${goal.toLocaleString()}${isInfinite ? " (Infinite)" : ""}`;
  // };
